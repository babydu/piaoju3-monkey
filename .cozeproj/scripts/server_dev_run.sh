...
install_python_deps() {
  ...
  if ! python3 -c "import cv2" 2>/dev/null; then
    echo "安装opencv-python-headless和rapidocr-onnxruntime..."
    pip3 install --quiet opencv-python-headless rapidocr-onnxruntime 2>/dev/null || true
    # 安装系统依赖（如果需要）
    if ! python3 -c "import cv2" 2>/dev/null; then
      apt-get update -qq && apt-get install -y -qq libgl1 libglib2.0-0 2>/dev/null || true
    fi
    echo "Python OCR依赖安装完成"
  fi
}
